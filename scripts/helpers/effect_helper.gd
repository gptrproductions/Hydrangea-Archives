...
enum {SHINGLES, SPORDERR, DAMASCUS, FLINCH_KINETIC, AAAAAA}
...
static func get_effect(effect_name : int):
	match effect_name:
		...
		DAMASCUS: return load("res://effects/damascus/damascus.tscn")

# Func always starts with the role it has.
static func start(effect_name: int, role: Hud.role, value = 0):
	...
	if role == Hud.role.PLAYER: 
		...
		gameplay.player_bar.get_node("stacks").add_child(instance)
	else: 
		instance.get_node("icon").texture_normal = load("res://assets/icons/effects/icon_under_red.webp")
		gameplay.enemy_bar.get_node("stacks").add_child(instance)
	# Tooltip signals.
	...
	instance.get_node("description_normal").visible = false
	if is_instance_valid(instance.get_node("description_dumb")): instance.get_node("description_dumb").visible = false
	instance.add_to_group("effects")
	instance.start(role, value)
...
static func get_tooltip(target: TextureProgressBar, enabled : bool = true):
	var string : String
	string = "description_normal" if false else "description_dumb" ##CHANGE THIS TO AN OPTION
	if !is_instance_valid(target.get_node("description_dumb")): string = "description_normal"
	if System.input_disabled == true: enabled =  false
	if enabled: 
		target.get_node(string).visible = true
		target.get_node(string).modulate = Color.TRANSPARENT
	
	else: if enabled: target.get_node(string).modulate = Color.WHITE
	var color : Color
	var tween = target.create_tween()
	if enabled: color = Color.WHITE
	else: color = Color.TRANSPARENT
	tween.tween_property(target.get_node(string), "modulate", color, 0.075)
