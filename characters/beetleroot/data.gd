extends Node
class_name Character_Beetleroot

# Reference to the scene.
static var scene : PackedScene = preload("res://characters/beetleroot/beetleroot.tscn")
var role : Hud.role = Hud.role.ENEMY

var lore : Dictionary = {
	"skill1" : {"name" : "Beetroot Beater", "description" : "Spins around and slaps the oponnent using his beetroot, dealing minor damage.", "button" : load("res://characters/beetleroot/assets/button_skill1.webp"), "type": Combat.move_type.ATTACK},
	"skill2" : {"name" : "Backflip Backshot", "description" : "Performs a backflip that causes Beetleroot's beetroot to strike the opponent, dealing moderate damage. ", "button" : load("res://characters/beetleroot/assets/button_skill2.webp"), "type": Combat.move_type.ATTACK},
	"skill3" : {"name" : "Backflip Backshot Pro", "description" : "Performs a backflip that causes Beetleroot's beetroot to strike the opponent, and ricochet to the next one, dealing damage.", "button" : load("res://characters/alligator/assets/button_skill3.webp"), "type": Combat.move_type.ATTACK},
	"skill4" : {"name" : "SLAM DUNK", "description" : "Alligator smashes his beetroot against the opponent, dealing massive damage.", "button" : load("res://characters/beetleroot/assets/button_ultimate.webp"), "type": Combat.move_type.ATTACK},
...
var stats : Dictionary = {
	...
	"volume" : 1,
	"type" : Hud.mindset.ONEIRIC,
	"rarity" : 0,
	"death_type" : Character.death_type.INSTANT,
	
	# Active character hud avatar
	"profile" : preload("res://characters/beetleroot/assets/avatar.webp"),
	...
	"resistance" : 10, # How much in percentage does the character resist taking damage.
	...
	if is_instance_valid(get_tree().root.get_node("hud")):
		...
		match(skill):
			...
			Hud.skills.SKILL_4:
				...
				flinch = max(((self_reference["flinch"]) * 0.7), int(opponent_reference["max_flinch"] * 0.01))
